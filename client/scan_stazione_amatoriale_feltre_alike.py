...
def scan_stazione_amatoriale_feltre_alike(last_seen_timestamp, server, save=True, log=True):
  ...
  import requests
  response = requests.get(weather_station_url).content
  soup = BeautifulSoup(response, 'lxml')

  #for tbody in soup.find_all('div', id="currentTimestampValue"):
  for tbody in soup.find_all('div', id="currentConditionsBigDiv"):
  ...
  try:
    timestamp_list = tree.xpath('//*[@id="currentTimestampValue"]')
    path="//div"
